...
/* Toast notification.
 *   toast("hello")                          → plain, accent icon (info)
 *   toast("fail", {tone:"err"})             → red tint + alert icon
 *   toast("done", {icon:"check",tone:"ok"}) → custom icon
 *   toast("slow thing...", {id:"poi",long:true,icon:"loader"}) → updatable by id
 *     - returns the DOM node so callers can dismiss/replace it
 */
function toast(m, opts) {
  opts = opts || {};
  var tone = opts.tone || "";                    // '', 'ok', 'warn', 'err', 'info'
  ...
  /* If an id is provided, reuse/replace any existing toast with that id. */
  if (opts.id) {
