...
function renderPOIs(rd, elements) {
  ...
  for (var ci = 0; ci < maxShow; ci++) {
    ...
    popHtml += 'style="width:100%;padding:8px;border-radius:8px;background:' + (onRoute ? "var(--bg3)" : "var(--acc)") + ';color:' + (onRoute ? "var(--t1)" : "#060a12") + ';border:' + (onRoute ? "1px solid var(--brd2)" : "none") + ';font-weight:700;font-size:12px;font-family:var(--f);cursor:pointer">';
...
function showResult() {
  ...
  h += '<button class="btn btn-beta" id="rexp" style="flex:1;padding:12px 14px">\u2192 Google Maps</button>';
  h += '</div>';
  h += '<div style="text-align:center;font-size:10px;color:var(--t3);margin-top:4px;line-height:1.4">';
  h += 'Open route in your preferred navigation app for turn-by-turn directions</div>';

  h += '<div class="ar" style="margin-top:8px">';
  h += '<button class="btn btn-log-inline" id="rlog" style="flex:1">\u2261 Log This Route to Logbook</button>';
  h += '</div>';
